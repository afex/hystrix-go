...
// Go runs your function while tracking the health of previous calls to it. If
// your function begins slowing down or failing repeatedly, we will block new
// calls to it for you to give the dependent service time to repair.
//
// Define a fallback function if you want to define some code to execute during
// outages.
func Go(name string, run runFunc, fallback fallbackFunc) chan error {
	...
	// dont have methods with explicit params and returns let data come in and
	// out naturally, like with any closure explicit error return to give place
	// for us to kill switch the operation (fallback)
	...
	go func() {
		...
		// Circuits get opened when recent executions have shown to have a high
		// error rate. Rejecting new executions allows backends to recover, and
		// the circuit will allow new traffic when it feels a healthly state
		// has returned.
		if circuit.IsOpen() {
		...
		//
		// When requests slow down but the incoming rate of requests stays the
		// same, you have to run more at a time to keep up. By controlling
		// concurrency during these situations, you can shed load which
		// accumulates due to the increasing ratio of active commands to
		// incoming requests.
		select {
		case ticket := <-tickets:
